...
def load_params(params_path: str) -> dict:
    ...
    try:
        ...
        if params is None:
            logger.error('Parameters file %s is empty or did not contain a mapping', params_path)
            raise ValueError(f'Parameters file {params_path} is empty or invalid')
        if not isinstance(params, dict):
            logger.error('Parameters file %s did not contain a mapping (expected dict)', params_path)
            raise ValueError(f'Parameters file {params_path} did not contain a mapping')
